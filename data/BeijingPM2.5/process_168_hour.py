...
# ###########################################################################################################
train_df.to_csv('Train_set_168_hour_full.csv', sep=' ', encoding='utf-8', index=False, header=True)
test_df.to_csv('Test_set_168_hour_full.csv', sep=' ', encoding='utf-8', index=False, header=True)

# ###########################################################################################################
#  REMOVE UNWANTED COLUMNS, NORMALISE AND WRITE TO DISK
#  -- WE REMOVE THE DIFF VERSION OF THE TARGET AS THIS DATA IS GENERALLY STATIONARY
# ###########################################################################################################
features = train_df.columns.tolist()
unwanted = ["No", "year", "month", "day", "TARGET_pm2.5_168_DIFF"] 
...
nzr.write_field_config(config, target_col, 'Target_168_nzr_config.yaml')

train_df_norm = nzr.normalize(train_df2, config, ['N','S','E','W'])
test_df_norm = nzr.normalize(test_df2, config, ['N','S','E','W'])

train_df_norm.to_csv('Train_set_168_hour_normalised.csv', sep=' ', encoding='utf-8', index=False, header=False)
test_df_norm.to_csv('Test_set_168_hour_normalised.csv', sep=' ', encoding='utf-8', index=False, header=False)
