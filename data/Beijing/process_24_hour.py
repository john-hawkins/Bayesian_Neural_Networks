...
# ###########################################################################################################
features = train_df.columns.tolist()
unwanted = ["No", "year", "month", "day", 'TARGET_pm2.5_24_VALUE', "TARGET_pm2.5_24_PROP_DIFF" ]
for x in unwanted : 
    features.remove(x)
...
nzr.write_field_config(config, target_col, 'sets/Target_24_nzr_config_diff.yaml')

train_df_norm = nzr.normalize(train_df2, config, ['N','S','E','W'])
test_df_norm = nzr.normalize(test_df2, config, ['N','S','E','W'])

train_df_norm.to_csv('sets/Train_24_hour_diff.csv', sep=' ', encoding='utf-8', index=False, header=False)
test_df_norm.to_csv('sets/Test_24_hour_diff.csv', sep=' ', encoding='utf-8', index=False, header=False)
...
# ###########################################################################################################
features = train_df.columns.tolist()
unwanted = ["No", "year", "month", "day", 'TARGET_pm2.5_24_VALUE', "TARGET_pm2.5_24_DIFF" ]
for x in unwanted :
    features.remove(x)
...
target_col = "TARGET_pm2.5_24_PROP_DIFF"
nzr.write_field_config(config, target_col, 'sets/Target_24_nzr_config_prop_diff.yaml')

train_df_norm = nzr.normalize(train_df2, config, ['N','S','E','W'])
test_df_norm = nzr.normalize(test_df2, config, ['N','S','E','W'])

train_df_norm.to_csv('sets/Train_24_hour_prop_diff.csv', sep=' ', encoding='utf-8', index=False, header=False)
test_df_norm.to_csv('sets/Test_24_hour_prop_diff.csv', sep=' ', encoding='utf-8', index=False, header=False)
